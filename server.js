...
import { D, ALIASES, CHAMP_COUNT } from './dataset.js';
...
function validateContext(raw={}){
  const roles = ['top','jungle','mid','adc','support','sup','auto'];
  ...
  const comfortMap = raw?.playerProfile?.comfort && typeof raw.playerProfile.comfort === 'object'
    ? raw.playerProfile.comfort
    : {};
  const comfort = Object.fromEntries(
    Object.entries(comfortMap).map(([k,v]) => [champKey(k), clamp(Number(v)||0,0,1000)])
  );
...
function damageProfile(champs=[]){
  return champs.reduce((acc, key) => {
    acc.ap += D[key]?.damage?.ap || 0;
    acc.ad += D[key]?.damage?.ad || 0;
    return acc;
  }, { ap: 0, ad: 0 });
}
...
function detectEnemyWinCondition(enemies=[]){
  ...
  const tags = enemyKeys.reduce((acc,k)=>{
  ...
  } else if (tags.scaling >= 7 && tags.teamfight >= 6){
    label = 'scale / teamfight';
    explanation = 'A comp inimiga quer alongar o jogo e lutar 5v5 em objetivos maiores.';
  } else if (tags.sideLane >= 6){
    label = 'side lane';
    explanation = 'A comp inimiga quer pressão lateral e quebrar a estrutura do mapa.';
  }
  return { label, explanation, tags };
}

function counterPlan(enemyWin){
  switch (enemyWin.label){
    case 'dive / pick':
      return {
        title: 'Neutralizar dive / pick',
        why: 'O inimigo quer achar pick em fog e forçar engage curto.',
        bullets: [
          'Jogue com visão antes de avançar side lane.',
          'Segure recursos para peel e counter-engage.',
          'Evite flancos sem informação.'
        ]
      };
    case 'poke / siege':
      return {
        title: 'Quebrar poke / siege',
        why: 'O inimigo quer te desgastar antes do objetivo.',
        bullets: [
          'Preserve HP e entre com wave empurrada.',
          'Busque flank ou engage em janela curta.',
          'Evite facecheck em choke.'
        ]
      };
    case 'scale / teamfight':
      return {
        title: 'Punir scaling',
        why: 'O inimigo escala melhor se o jogo alongar.',
        bullets: [
          'Jogue por tempo e prioridade cedo.',
          'Acelere pickoffs e objetivos antes do spike deles.',
          'Evite 5v5 ideal no timing forte inimigo.'
        ]
      };
    case 'side lane':
      return {
        title: 'Responder side lane',
        why: 'O inimigo quer te quebrar no mapa lateral.',
        bullets: [
          'Controle visão nas laterais.',
          'Force objetivo quando o split estiver mostrado.',
          'Não reaja tarde ao side pressure.'
        ]
      };
    default:
      return {
        title: 'Plano geral',
        why: 'A comp inimiga ainda não mostra uma condição única forte.',
        bullets: [
          'Jogue por informação e tempo de wave.',
          'Priorize objetivos com número e visão.',
          'Não force fight sem condição clara.'
        ]
      };
  }
}
...
function computeConfidence(metrics, ctx){
  ...
  if (revealed <= 2) conf -= 12;
  else if (revealed <= 6) conf -= 5;
...
function categoryScores(champ, metrics){
  return {
    blind: clamp(metrics.comp*0.28 + metrics.meta*0.18 + metrics.riskInverse*0.28 + metrics.lanePrio*0.16 + getProfile(champ,'safe')*4, 0, 100),
    punish: clamp(metrics.matchup*0.42 + metrics.pick*0.18 + metrics.synergy*0.10 + metrics.lanePrio*0.10 + getProfile(champ,'counterpick')*5, 0, 100),
    comfort: clamp(metrics.comfort*0.50 + metrics.matchup*0.15 + metrics.meta*0.10 + metrics.riskInverse*0.10 + getProfile(champ,'safe')*5, 0, 100),
...
function whyThisWinsGame(champ, metrics, ctx){
  ...
  if (enemyWin.label === 'dive / pick' && (getTag(champ,'antiDive') >= 2 || getTag(champ,'peel') >= 2)) {
    return 'Ganha jogo porque reduz o valor do engage inimigo e força lutas mais previsíveis.';
  }
  if (enemyWin.label === 'poke / siege' && getTag(champ,'engage') >= 2) {
    return 'Ganha jogo porque cria janela de engage antes do setup inimigo ficar perfeito.';
  }
  if (enemyWin.label === 'scale / teamfight' && (metrics.lanePrio >= 60 || metrics.matchup >= 55)) {
    return 'Ganha jogo porque acelera o mapa antes do 5v5 ideal da comp inimiga.';
  }
...
function plan10(champ, metrics, ctx){
  ...
  if (enemyWin.label === 'dive / pick') return `${base} Guarde recursos para responder engage e priorize visão profunda antes de contestar rio.`;
  if (enemyWin.label === 'poke / siege') return `${base} Preserve HP, entre em objetivo com wave empurrada e procure flank antes da fight frontal.`;
  if (enemyWin.label === 'scale / teamfight') return `${base} Acelere prio de lane e tente criar janela de pick antes do 5v5 ideal deles.`;
  if (enemyWin.label === 'side lane') return `${base} Controle sides cedo e force objetivos quando o split estiver mostrado.`;
  if (metrics.lanePrio >= 65) return `${base} Priorize pressão de lane e primeiro movimento para o rio.`;
...
function scoreCandidate(champ, ctx){
  ...
  const teamfight = clamp(40 + getTag(champ,'teamfight')*20, 0, 100);
  ...
  const score = matchup*weights.matchup + synergy*weights.synergy + comp*weights.comp + damageFit*weights.damageFit + engageFit*weights.engageFit + lanePrio*weights.lanePrio + comfort*weights.comfort + meta*weights.meta - risk;

  const metrics = { matchup, synergy, comp, damageFit, engageFit, lanePrio, comfort, meta, risk, riskInverse, pick, teamfight };
...
function filterCandidates(ctx){
  const keys = Object.keys(D).filter(k => !ctx.bans.includes(k));
  if (ctx.role === 'auto') return keys;
  return keys.filter(k => (D[k]?.role || 'mid') === ctx.role);
}
function topBy(results, key){
  return [...results].sort((a,b) => (b.categoryScores?.[key] || 0) - (a.categoryScores?.[key] || 0)).slice(0,3);
}
function buildPayloadForChamp(champKey){
  const c = D[champKey];
  if (!c) return null;
  return {
    champion: titleFromKey(champKey),
    role: c.role || '',
    cls: c.cls || '',
    spells: c.f || [],
    starter: c.ini || '',
...
app.get('/champions', (req,res) => res.json(Object.keys(D).map(k => titleFromKey(k)).sort()));

function finishAnalyze(startedAt, ok=true){
  const ms = Date.now() - startedAt;
  if (!ok) stats.analyzeErrors += 1;
...
app.post('/analyze', (req,res) => {
  ...
  try {
    ...
    const cacheKey = 'analyze:' + stableStringify(ctx) + ':' + (req.body?.selectedPick || '');
    const cached = cacheGet(cacheKey);
    if (cached) {
      return res.json({ ...cached, responseMs: finishAnalyze(startedAt, true), cached: true });
    }
    ...
    const cp = counterPlan(enemyWinCondition);
    const recommendation = {
      ...
      details: picks[0]?.whyWins || 'Revele mais picks para aumentar a confiança.'
    };
    const payload = {
      ctx,
      picks,
      ...
      recommendation,
      summary: { enemyWinCondition, counterPlan: cp }
    };
    cacheSet(cacheKey, payload);
    res.json({ ...payload, responseMs: finishAnalyze(startedAt, true), cached: false });
  } catch (e) {
    finishAnalyze(startedAt, false);
    res.status(500).json({ error: e.message || 'internal_error' });
  }
});

app.post('/instant-pick', (req,res) => {
  const startedAt = Date.now();
  try {
    const raw = req.body?.context || {};
    const ctx = validateContext({ ...raw, playerProfile: req.body?.playerProfile || {} });
    const candidates = filterCandidates(ctx);
    const bestPick = candidates.map(ch => scoreCandidate(ch, ctx)).sort((a,b) => b.score - a.score)[0] || null;
    res.json({
      bestPick,
      autoContext: { available: false, reason: 'manual_only' },
      liveClient: false,
      responseMs: finishAnalyze(startedAt, true)
    });
  } catch (e) {
    finishAnalyze(startedAt, false);
    res.status(500).json({ error: e.message || 'instant_pick_error' });
  }
});
